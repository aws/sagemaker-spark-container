...
class _SparkProcessorBase(ScriptProcessor):
    ...
    def __init__(
        ...
        if framework_version and framework_version not in framework_versions:
            raise ValueError(
                "framework_version {} is not one of {}".format(framework_version, [key for key in framework_versions])
            )
    ...
    def _stage_configuration(self, configuration):
        ...
        S3Uploader.upload_string_as_file_body(body=serialized_configuration, desired_s3_uri=s3_uri)
    ...
    def _stage_submit_deps(self, submit_deps, input_channel_name):
        ...
        with tempfile.TemporaryDirectory() as tmpdir:
            ...
            if os.listdir(tmpdir):
                ...
                S3Uploader.upload(
                    local_path=tmpdir, desired_s3_uri=input_channel_s3_uri, session=self.sagemaker_session
                )
    ...
    def _check_history_server(self):
        # ping port 18080 to check history server is up
    ...
    def _is_history_server_started(self):
        try:
            response = urllib.request.urlopen("http://localhost:18080")
...
class _HistoryServer:
    ...
    def __init__(self, env_variables, image_uri):
        ...
        self.docker_login_command = self._get_docker_login_cmd()
        self.run_history_server_command = self._get_run_history_server_cmd()

    def run(self):
        print("Starting history server...")
        subprocess.Popen(self.docker_login_command, shell=True).wait()
    ...
    def _get_docker_login_cmd(self):
        ...
        cmd = "$(aws ecr get-login --region {} --registry-ids {} --no-include-email)".format(region, account_id)
        return cmd
