...
def test_run_with_event_log_dir(mock_open_file,
    ...
    publisher = SparkEventLogPublisher(None, SPARK_LOCAL_EVENT_LOG_DIR, copy_interval=5)
    publisher.start()
...
def test_run_with_spark_events_s3_uri(mock_open_file):
    publisher = SparkEventLogPublisher(SPARK_EVENT_LOG_S3_URI, SPARK_LOCAL_EVENT_LOG_DIR, copy_interval=5)
    publisher.start()
